"""
Early-Exit ResNet-18 전용 Trainer

출력이 3개 (ee1, ee2, main) 인 EE 모델에서만 사용.
가중 합산 손실:  loss = w1·L(ee1) + w2·L(ee2) + w3·L(main)
"""
...
def train_one_epoch(model, loader, optimizer, criterion, device,
    ...
    for images, labels in loader:
        ...
        optimizer.zero_grad()
        out1, out2, out_main = model(images)

        loss = (w1 * criterion(out1,     labels)
              + w2 * criterion(out2,     labels)
              + w3 * criterion(out_main, labels))
        loss.backward()
        optimizer.step()
        ...
        correct3 += out_main.argmax(1).eq(labels).sum().item()
