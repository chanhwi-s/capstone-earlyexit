...
"""
Early Exit Head
AdaptiveAvgPool2d(1) → Flatten → Linear(in_channels, num_classes)
"""
...
# ── ResNet-50 with Early Exit ────────────────────────────────────────────────

"""
plain_resnet50과 동일한 backbone에 exit head를 layer1, layer3 출력 직후에 부착.

Exit point별 채널 수 (224x224 입력 기준):
  exit1  : layer1 출력  256  x 56 x 56   (64  * expansion 4)
  exit2  : layer3 출력  1024 x 14 x 14   (256 * expansion 4)
  main   : layer4 출력  2048 x  7 x  7   (512 * expansion 4)

forward(x, threshold=None) 동작:
  threshold=None  → 학습 모드. 세 exit의 logit을 리스트로 반환.
...
class ResNet_EE(nn.Module):
    def __init__(self, block, layers, num_classes=1000):
        ...
        # Residual layers
        # layer1 출력: 256 ch  ← exit1
        # layer2 출력: 512 ch
        # layer3 출력: 1024 ch ← exit2
        # layer4 출력: 2048 ch ← main
        ...
        self.exit2   = ExitHead(256 * block.expansion, num_classes)  # 1024
