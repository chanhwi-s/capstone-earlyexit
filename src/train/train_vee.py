...
사용법:
  cd src
  python train/train_vee.py                          # CIFAR-10 (기본값)
  python train/train_vee.py --dataset imagenet       # ImageNet
  python train/train_vee.py --dataset imagenet --epochs 60 --batch-size 128
...
주요 인자:
  ...
  --seed        랜덤 시드
  --w-exit      exit head loss 가중치 (기본: train.yaml w1)
  --w-main      main head loss 가중치 (기본: train.yaml w3)

결과 저장 위치:
  experiments/exp_.../train/vee_resnet18/
...
def build_config(args):
    ...
    if args.w_exit     is not None: cfg["train"]["w1"]          = args.w_exit
    if args.w_main     is not None: cfg["train"]["w3"]          = args.w_main
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="VEE ResNet-18 학습")
    ...
    parser.add_argument("--seed",       type=int,   default=None,
                        help="랜덤 시드")
    parser.add_argument("--w-exit",     type=float, default=None,
                        help="exit head loss 가중치 (기본: train.yaml w1)")
    parser.add_argument("--w-main",     type=float, default=None,
                        help="main head loss 가중치 (기본: train.yaml w3)")
