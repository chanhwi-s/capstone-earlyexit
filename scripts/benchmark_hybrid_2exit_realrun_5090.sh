...
#    N_SAMPLES            랜덤 샘플 수 (기본: 5000)
#    SEED                 랜덤 시드 (기본: 42)
#    BATCH_SIZES          2-exit grid 탐색 배치 크기 목록 (기본: 8 16 32 64 128 256 512)
#                         — minimum은 baseline과 동일한 8, 이후 2배씩 증가
#    TIMEOUT_MS           ms (기본: 1 2 5 10 20 50 100 200 500)
#    WARMUP               웜업 샘플 수 (기본: 50)
#    BASELINE_BATCH_SIZE  PlainViT 기준선 배치 크기 (기본: 8, MLPerf/Triton industry standard)
...
SEED="${SEED:-42}"
# 2-exit grid: minimum=8 (baseline와 동일), 이후 2배 등비
BATCH_SIZES="${BATCH_SIZES:-8 16 32 64 128 256 512}"
TIMEOUT_MS="${TIMEOUT_MS:-1 2 5 10 20 50 100 200 500}"
WARMUP="${WARMUP:-50}"
# MLPerf Inference Server / NVIDIA Triton dynamic batching industry standard
...
echo "  BATCH_SIZES (grid) : $BATCH_SIZES"
echo "  TIMEOUT_MS         : $TIMEOUT_MS"
echo "  WARMUP             : $WARMUP"
echo "  BASELINE_BATCH_SIZE: $BASELINE_BATCH_SIZE  (industry standard)"
echo "============================================"
echo ""
