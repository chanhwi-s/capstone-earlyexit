#!/usr/bin/env bash
# ============================================================
#  train_vit_2exit_5090.sh  —  RTX 5090 서버용 2-Exit ViT 학습 파이프라인
#
#  Exit 구성: B8 (early exit) + B12 (main exit)
#
#  실행 순서:
#    0. 타임스탬프 기반 실험 디렉토리 생성
#    1. SelectiveExitViT (exit_blocks=[8, 12]) exit head fine-tuning
#       → {EXP_DIR}/train/ee_vit_2exit/
#
#  데이터셋: ImageNet (224×224, 1000 classes)
#  모델    : ViT-B/16 (pretrained backbone frozen, exit head 2개만 학습)
...
#  사용법:
#    cd <project_root>
#    bash scripts/train_vit_2exit_5090.sh
#
#  백그라운드 실행 (권장):
#    nohup bash scripts/train_vit_2exit_5090.sh > vit_2exit_train.log 2>&1 &
#    tail -f vit_2exit_train.log
# ============================================================
...
echo "============================================"
echo "  2-Exit ViT-B/16 Fine-tuning (B8 + B12)"
echo "  Project root  : $PROJECT_ROOT"
echo "  Exp dir       : experiments/$EXP_NAME"
echo "  Exit blocks   : [8, 12]"
...
EXP_NAME    : $EXP_NAME
EXP_DIR     : $EXP_DIR
Model       : SelectiveExitViT-B/16 (exit_blocks=[8, 12], backbone frozen)
...
# ── 1. SelectiveExitViT (2-exit) 학습 ────────────────────────
echo "[1/1] 2-Exit ViT-B/16 fine-tuning (exit_blocks=[8, 12]) ..."
echo "      Args: --exit-blocks 8 12 $EXTRA_ARGS"
python train/train_vit_selective.py --exit-blocks 8 12 $EXTRA_ARGS
...
echo ""
echo "============================================"
echo "  2-Exit ViT Training Done!"
echo "  Exp dir    : experiments/$EXP_NAME"
echo "  Checkpoints:"
echo "    experiments/$EXP_NAME/train/ee_vit_2exit/checkpoints/best.pth"
echo "    experiments/$EXP_NAME/train/ee_vit_2exit/checkpoints/final.pth"
echo "  Log:"
echo "    experiments/$EXP_NAME/train/ee_vit_2exit/train_log.csv"
echo "============================================"
