#!/usr/bin/env bash
# ============================================================
#  benchmark_hybrid_3exit_realrun_5090.sh
#  LUT 시뮬레이션이 아닌 실제 GPU 실행 기반 3-exit 하이브리드 벤치마크.
#  ImageNet val에서 N개 랜덤 샘플링 → grid search.
#
#  사용법:
#    bash scripts/benchmark_hybrid_3exit_realrun_5090.sh --threshold 0.80
#    N_SAMPLES=2000 bash scripts/benchmark_hybrid_3exit_realrun_5090.sh --threshold 0.80
#    VARIANTS="A C" BATCH_SIZES="1 4 8" TIMEOUT_MS="1 2 5" \
#      bash scripts/benchmark_hybrid_3exit_realrun_5090.sh --threshold 0.80
#
#  환경 변수:
...
echo "============================================"
echo "  3-exit Real-Run Benchmark  (RTX 5090)"
...
cd "$SRC_DIR"
python benchmark/hybrid_vit_3exit_realrun.py \
    --data-root   "$DATA_ROOT" \
    --n-samples   "$N_SAMPLES" \
    --seed        "$SEED" \
    --variants    $VARIANTS \
